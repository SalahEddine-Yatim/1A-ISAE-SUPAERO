 ...
 * @file solver.c
 * 
 * @brief A simple program to solve maps
 * 
...
char* build_plan(cell_map* p_map_cell , cell_map* p_initial_cell){
    ...
    while (current_cell != p_initial_cell){
        ...
        //Adding each move to the plan by going backward
...
bool equal_maps(char* p_map1 , char* p_map2 , int height , int width){
    ...
    for(int i=0 ; i < height ; i++ ){
        ...
        for(int j=0 ; j < width ; j++ ){
            ...
            //if there is a different caractere the not the same map 
...
char* solve_function(FILE *p_file) {
    ...
    //Initializing the queues and the explored list 
    ...
    while (search_queue->p_head->p_next_map != NULL){
        ...
        for (int i=0 ; i < 4 ; i++){
            ...
            //Si mvmt impossible on a pas d'espace a liberer on a pas utiliser malloc dans move (p_current_map == p_new_map)
            if(p_current_map != p_new_map){
                ...
                if (is_in_list_map(explored_list, p_new_map,  height , width)){
                    ...
                    //Creating new cell for the new generated map 
    ...
    if(is_a_winnig_map(p_current_cell->p_map , height , width)){
        ...
        //Building the winnig plan 
        //We can also have the lenght of the plan it is  p_current_cell->depth 
