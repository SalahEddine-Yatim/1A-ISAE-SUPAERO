...
object NilInt extends ListInt {
  def head = ???
  def tail = NilInt
  def isEmpty = true
  def size = 0
  def isTheSameAs(that: ListInt) = that == this 
}
...
case class ConsInt(val head: Int, val tail: ListInt) extends ListInt {
  ...
  def isTheSameAs(that: ListInt) = 
    that != NilInt && that.head == this.head && this.tail.isTheSameAs(that.tail)
}

object ConsInt {
  def apply(head: Int, tail: ListInt) =
    new ConsInt(head, tail)
}
